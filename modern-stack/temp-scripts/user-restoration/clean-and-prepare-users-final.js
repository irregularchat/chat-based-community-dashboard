...
// Clean up the names by removing "Envelope from: " prefix and smart quotes
const cleanedUsers = rawData.users.map(user => {
    let cleanName = user.name;
    
    // Remove "Envelope from: " prefix
    if (cleanName.startsWith('Envelope from: ')) {
        cleanName = cleanName.substring('Envelope from: '.length);
    }
    
    // Remove smart quotes (Unicode 8220 and 8221) or regular quotes
