/**
 * AI Integration Module for Signal Bot (TypeScript version)
 * 
...
interface AIConfig {
  ...
  isAdmin?: (userIdentifier: string, groupId?: string) => boolean;
  isModerator?: (userIdentifier: string, groupId?: string) => boolean;
  communityContext?: CommunityContext;
  wikiUrl?: string;
  forumUrl?: string;
  newsSummaries?: Map<string, NewsSummary>;
}
...
interface CommandRegistry {
  ...
  all: CommandInfo[];
  categorized: Record<string, CommandInfo[]>;
...
interface SafeCommandResult {
  ...
  command?: string;
  executedBy?: string;
  needsPermission?: 'admin' | 'moderator';
  blocked?: boolean;
  error?: boolean;
}

interface UserAIPreference {
  provider: 'openai' | 'localai';
  timestamp: number;
  lastMessage: string;
}
...
export class AIIntegration {
  ...
  private localAiUrl?: string;
  private localAiApiKey?: string;
  private prisma?: PrismaClient;
  private plugins: Map<string, CommandPlugin>;
  private isAdmin: (userIdentifier: string, groupId?: string) => boolean;
  private isModerator: (userIdentifier: string, groupId?: string) => boolean;
  private communityContext: CommunityContext;
  private wikiUrl: string;
  private forumUrl: string;
  private newsSummaries: Map<string, NewsSummary>;
  private userAiPreference: Map<string, UserAIPreference>;
  ...
  constructor(config: AIConfig = {}) {
    ...
    this.prisma = config.prisma || undefined;
    this.plugins = config.plugins || new Map<string, CommandPlugin>();
    ...
    this.newsSummaries = config.newsSummaries || new Map<string, NewsSummary>();
    
    // Thread context tracking for multi-turn AI conversations
    this.userAiPreference = new Map<string, UserAIPreference>();
   ...
   */
  async handleOpenAIChat(context: CommandContext): Promise<string> {
    if (!this.openai) {
    ...
    let contextInfo = '';
    let responseMode: 'general' | 'command' | 'community' = 'general';
    
    // Define AI prefix based on context
    const getAiPrefix = (mode: string) => 
      mode === 'command' ? 'OpenAI [Commands]:' : 
      mode === 'community' ? 'OpenAI [Community]:' : 
      'OpenAI:';
   ...
   */
  async handleLocalAIChat(context: CommandContext): Promise<string> {
    ...
    // Build the AI prompt with context (similar to OpenAI implementation)
    const messages: Array<{ role: 'system' | 'user'; content: string }> = [
   ...
   */
  async summarizeUrl(url: string, context: CommandContext): Promise<string> {
    if (!this.openai) {
   ...
   */
  async getAIDatabaseContext(query: string, context: CommandContext): Promise<DatabaseContext> {
    ...
    try {
      ...
      if (query.match(/event|meeting|meetup|happening|schedule|when is/i)) {
        ...
        dbContext.events = events.map(e => ({
          ...
          name: e.name,
          start: e.eventStart,
          end: e.eventEnd || undefined,
          location: e.eventLocation || undefined,
      ...
      // Additional queries for links and news...
   ...
   */
  async safeCommandExecutor(
    commandName: string, 
    args: string[], 
    context: CommandContext, 
    aiProvider: string = 'ai'
   ...
   */
  getCommandRegistry(context: CommandContext): CommandRegistry {
    ...
    const categorized: Record<string, CommandInfo[]> = {};
   ...
   */
  private getCommandUsage(name: string): string {
   ...
   */
  private getCommandExamples(name: string): string[] {
   ...
   */
  getUserAIPreference(context: CommandContext): UserAIPreference | undefined {
    const threadKey = `${context.groupId || 'dm'}:${context.sourceNumber}`;
   ...
   */
  setUserAIPreference(context: CommandContext, provider: 'openai' | 'localai', message: string): void {
    const threadKey = `${context.groupId || 'dm'}:${context.sourceNumber}`;
    this.userAiPreference.set(threadKey, {
      provider,
      timestamp: Date.now(),
