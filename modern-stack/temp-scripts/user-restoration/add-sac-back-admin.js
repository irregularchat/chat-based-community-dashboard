...
async function addSacBackAsAdmin() {
  console.log('🚨 PRIORITY: Adding Sac back to Counter UxV as admin');
  console.log('=' + '='.repeat(50));
  
  const counterUxvGroupId = 'hBnM2KMgzgUp7J9VoKSMBkfxkCE8eSh+K8FjM07Lt+U=';
  const sacPhoneNumber = '+12247253276'; // From the logs and env
  ...
  return new Promise((resolve, reject) => {
    ...
    socket.connect(socketPath, () => {
      ...
      console.log(`📤 Adding Sac (${sacPhoneNumber}) as admin to Counter UxV...`);
          ...
          if (response.id && response.id.startsWith('add-sac-admin-')) {
            ...
            if (!resolved) {
              ...
              if (response.result !== undefined) {
                console.log('✅ Successfully added Sac as admin to Counter UxV!');
                resolve(true);
...
async function main() {
  try {
    ...
    console.log('\n🎉 Sac has been added back to Counter UxV as admin!');
  } catch (error) {
