    ...
    if (line.trim() === `- ${targetTimestamp} (2025-09-03T15:10:57.792Z)`) {
        ...
        for (let j = i - 1; j >= Math.max(0, i - 10); j--) {
            ...
            if (prevLine.includes('Envelope from:')) {
                ...
                // Handle both UUIDs and phone numbers
                const uuidMatch = prevLine.match(/Envelope from: "([^"]*)" ([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}) \(device: \d+\) to/);
                const phoneMatch = prevLine.match(/Envelope from: "([^"]*)" (\+\d+) \(device: \d+\) to/);
                
                let name, identifier;
                if (uuidMatch) {
                    name = uuidMatch[1];
                    identifier = uuidMatch[2];
                } else if (phoneMatch) {
                    name = phoneMatch[1];
                    identifier = phoneMatch[2];
                } else {
