...
async function sendHelpCommand() {
  console.log('ℹ️ Sending !help command to Entry/INDOC room...');
    ...
    socket.on('data', (data) => {
      ...
      try {
        ...
        for (const line of lines) {
          ...
          if (response.id && response.id.startsWith('help-test-')) {
...
async function main() {
  console.log('Signal Bot Help Command Test');
  ...
  try {
    await sendHelpCommand();
    console.log('\n🎉 Help command sent to Entry/INDOC room!');
    console.log('Check the Signal group to see if the bot responded.');
  } catch (error) {
    console.log('\n❌ Failed to send help command:', error.message);
    process.exit(1);
  }
}
